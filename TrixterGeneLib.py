...
class Blast_table(Table):
    ...
    def distance_col(self, model=None, my_model=False, **kwargs):  # Реализует расчёт значений в столбце
        ...
        if model == 'JC':
            D_JC = np.where((1 - ((pident * length) / np.maximum(qlen, slen)) / 100) < 0.75,
                            (-3 / 4) * np.log(1 + (-4 / 3) * (1 - ((pident * length) / np.maximum(qlen, slen)) / 100)),
                            np.nan)
    ...
    def blast_dbscan(self, epsilon, mu=1):
        def all_possible_clusters(no_reverse_repeats_df):  # Функция превращает все возможные кластеры во множества
            return no_reverse_repeats_df.groupby('qseqid').apply(
                lambda x: set(x['qseqid']).union(set(x['sseqid']))).tolist()

        def sets_to_clusters(all_possible_clusters,
                             connected_column):  # Функция объединяет все мн-ва, имеющие пересечения
            ...
            for name in connected_column:  # Итерация всех вершин, имеющих ребро
                ...
                all_possible_clusters = np.append(all_possible_clusters[~logical],
                                                  new_cluster)  # Срез массива, без слитых мн-в
        ...
        if mu == 1:
            ...
            actual_clusters_df = clustering_df[
                distances]  # В кластеры попадут только те элементы, у которых есть хотя бы 1 пара
            ...
            names = np.sort(np.array([actual_clusters_df.qseqid, actual_clusters_df.sseqid]),
                            axis=0)  # Сортировка всех имён
            # в столбцах, чтобы можно было удалить случаи A   B; B   A методом drop_duplicates
            no_reverse_repeats_df = pd.DataFrame(names.T,
                                                 columns=['qseqid', 'sseqid']).drop_duplicates()  # Больше нет строк
    ...
    def create_real_graph(self, cluster_name, gc_content_fasta=False):
        cluster_df = self.df[self.df['qseqid'].isin(self.clusters[0][cluster_name]) & self.df['sseqid'].isin(
            self.clusters[0][cluster_name])]
...
class Loci:  # rep_classes - array, chromosomes - array
    def __init__(self, df, rep_classes, chromosomes, some_loci=False):
        if not some_loci:
            ...
            self.intervals = {chrom: self.filtered_df.loc[(self.filtered_df.name_of_query_sequence == chrom)] for chrom
                              in self.chromosomes}
                ...
                for chrom in loci_object.intervals.keys():
                    if chrom in record.id:
                        for index, row in loci_object.intervals[chrom].iterrows():
                            my_fasta.write(
                                f">{chrom},{row['the_class_of_the_repeat']},{row['name_of_the_matching_interspersed_repeat']},{row['starting_position_of_match']}:{row['ending_position_of_match']}\n")
                            my_fasta.write(
                                str(record.seq[row.starting_position_of_match:row.ending_position_of_match]) + '\n')
                        print(chrom)
...
class Mobile_element:
    def __init__(self, sequence, death_prob=0.1, transp_prob=0.9, my_number=None, parent_name=None,
                 initial_name='hydra'):
...
class Time:
    def __init__(self, initial_name, sequence, death_prob=0.1, transp_prob=0.8, mutation_rate=0.05):
        ...
        self.mobile_graph.columns = ['name1', 'name2', 'distance', 'is_alive1', 'is_alive2', 'gc_content1',
                                     'gc_content2']
    ...
    def expansion(self, transp_limit, time_limit=None, transposition_dynamics=None, death_dynamics=None,
                  model_for_evolution=None):
        num_created = 1
        num_dead = 0
    ...
    def draw_graph(self, node_color=False):
        ...
        if node_color is False:
            ...
            nx.draw(self.mobile_graph_network, pos, with_labels=False, node_color=node_colors, node_size=10,
                    font_size=1)
        else:
